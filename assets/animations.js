...
(function () {
  ...
  function initHowPage() {
    ...
    function buildStatAnimation(stat) {
      ...
      if (emoji) {
        gsap.set(emoji, { scale: 0.6, rotation: -10, opacity: 0 });
        gsap.to(emoji, {
      ...
      if (num && num.dataset.count) {
        const target = parseFloat(num.dataset.count);
        const suffix = num.dataset.suffix || '';
        const obj = { v: 0 };
        num.textContent = '0' + suffix;
        // Initial state — small + faded, scales up alongside the count
        ...
        gsap.to(obj, {
          ...
          delay: 0.3,
          scrollTrigger: { trigger: stat, start: 'top 88%', once: true },
          onUpdate: () => { num.textContent = Math.floor(obj.v) + suffix; },
          onComplete: () => { num.textContent = target + suffix; }
        });
      } else if (num && num.textContent.trim() === '24/7') {
        num.textContent = '0';
        const obj = { v: 0 };
        gsap.set(num, { opacity: 0, scale: 0.85 });
        gsap.to(num, {
        ...
        gsap.to(obj, {
          ...
          onComplete: () => {
            ...
            gsap.to(span, { opacity: 1, duration: 0.4, ease: 'power2.out', delay: 0.15 });
          }
        });
      } else if (num) {
      ...
      // Heading "10X FASTER" etc. — slide in from left (instead of the
      // slow per-char rise which made the user wait ~2s before reading).
      if (label) {
        gsap.set(label, { opacity: 0, x: -18, filter: 'blur(4px)' });
        gsap.to(label, {
      ...
      // Description — fade up just behind the heading (delay 0.75s).
      if (desc) {
        gsap.set(desc, { opacity: 0, y: 14, filter: 'blur(3px)' });
        gsap.to(desc, {
